...
| `batch.rs` | main-line only; INV-2 blunders; 2 % noise floor; concurrency cap | **`white_blunder_is_measured_from_whites_perspective`**, `a_two_percent_drop_is_still_noise`, `in_flight_scales_with_threads_and_saturates_at_sixteen` |
...
| `widgets/board.rs`, `widgets/tree.rs`, `widgets/winrate.rs` | Click→`Point` mapping, board geometry, tree lane assignment and graph axis inversion — pure functions deliberately lifted out of `snapshot()` so they are testable at all; a graph click owns the `NodeId` so `set_cursor` can rebuild the projection (INV-10) | `hit_test_snaps_to_the_nearest_intersection`, `lanes_keep_the_main_line_on_zero`, **`deep_lines_do_not_recurse`** (a long game must not blow the stack), `a_white_blunder_is_not_a_black_blunder`, **`a_click_takes_an_owned_node_so_the_projection_can_be_rebuilt`** |
