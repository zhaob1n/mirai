...
| `widgets/board.rs`, `widgets/tree.rs`, `widgets/winrate.rs` | Click→`Point` mapping, board geometry, tree lane assignment and graph axis inversion — pure functions deliberately lifted out of `snapshot()` so they are testable at all; a graph click owns the `NodeId` so `set_cursor` can rebuild the projection (INV-10); the blunder strip does not span unanalysed gaps | `hit_test_snaps_to_the_nearest_intersection`, `lanes_keep_the_main_line_on_zero`, **`deep_lines_do_not_recurse`** (a long game must not blow the stack), `a_white_blunder_is_not_a_black_blunder`, **`a_gap_is_not_a_blunder_bar`**, **`a_click_takes_an_owned_node_so_the_projection_can_be_rebuilt`** |
