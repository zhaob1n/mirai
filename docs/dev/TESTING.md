...
| Engine will not start, no useful message in the GUI | KataGo's own log | `$XDG_DATA_HOME/mirai/katago-logs` (GUI, `Config::data_dir`; also `mirai-server` when an `[[engine]]` has no `log_dir`), `$TMPDIR/mirai-katago-logs` (`LocalEngineConfig` default), or `log_dir` in `server.toml`. `PermissionDenied … refusing` means the directory for a generated config is another user's or writable by others (`atomic::private_dir`) |
